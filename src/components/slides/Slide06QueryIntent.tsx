      ...
      <Content className="justify-start">
        <div className="grid w-full max-w-[1400px] grid-cols-[340px_1fr] items-start gap-[72px]">
          <div className="flex w-full flex-col gap-2.5">
            <div className="mb-2 font-mono text-[0.68rem] tracking-[0.05em] text-ink-dim">
              同一主题 · 多个视角并行提问
            </div>
            {perspectives.map((p) => (
              <div
                ...
                className={`rounded-[9px] border px-4 py-3.5 ${
                  p.active
                    ? 'border-[rgba(140,109,18,0.65)] bg-[rgba(140,109,18,0.07)]'
                    : 'border-line bg-bg-card'
                }`}
              >
                <div className="font-mono text-[0.66rem] text-ink-dim">{p.num}</div>
                <h4 className="my-1 mb-1 font-display text-[0.92rem]">{p.title}</h4>
                <p className="m-0 text-[0.7rem] leading-[1.4] text-ink-dim">{p.desc}</p>
              </div>
            ))}
          </div>
          <div className="flex flex-col gap-0">
            {rounds.map((round) => (
              <div key={round.label} className="mb-[22px] flex flex-col gap-2">
                <div className="mb-1.5 flex items-center gap-2 font-mono text-[0.62rem] text-bronze after:h-px after:flex-1 after:bg-line-soft after:content-['']">
                  {round.label}
                </div>
                <div className="flex items-start gap-3.5">
                  <div className="w-[70px] shrink-0 pt-2.5 text-right font-mono text-[0.66rem] text-ink-dim">
                    写作者 · Writer
                  </div>
                  <div className="rounded-xl border border-line bg-bg-card px-[18px] py-3.5 text-[0.78rem] leading-[1.45]">
                    {round.writer}
                  </div>
                </div>
                <div className="flex items-start gap-3.5">
                  ...
                  <div className="rounded-xl border border-[rgba(61,81,99,0.45)] bg-bg-card px-[18px] py-3.5 text-[0.78rem] leading-[1.45]">
                    {round.expert}
                    <div className="mt-[5px] font-mono text-[0.6rem] leading-[1.25] text-slate">
                      {round.source}
                    </div>
                  </div>
                </div>
