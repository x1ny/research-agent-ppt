      ...
      <Content>
        <div className="overflow-hidden rounded-xl border border-line bg-bg-card">
          <div className="flex items-center gap-4 border-b border-line px-8 py-6">
            ...
            <span className="h-2.5 w-2.5 rounded-full bg-line" />
            <span className="ml-3 font-mono text-[0.82rem] text-ink-dim">大纲编辑器 · 待确认</span>
          </div>
          <div className="flex min-h-[230px]">
            <div className="w-[38%] border-r border-line p-9">
              <div className="mb-2.5 flex items-center gap-[18px] rounded-lg px-[11px] py-[9px] text-[0.98rem] text-ink-dim">
                <span className="font-mono text-[0.85rem] text-ink-dim opacity-50">⋮⋮</span> 一、检查背景与依据
              </div>
              <div className="mb-2.5 flex items-center gap-[18px] rounded-lg border border-[rgba(166,51,42,0.35)] bg-seal-tint px-[11px] py-[9px] text-[0.98rem] font-semibold text-ink">
                <span className="font-mono text-[0.85rem] text-ink-dim opacity-50">⋮⋮</span> 二、检查范围与内容
              </div>
              <div className="mb-2.5 flex items-center gap-[18px] rounded-lg px-[11px] py-[9px] text-[0.98rem] text-ink-dim">
                <span className="font-mono text-[0.85rem] text-ink-dim opacity-50">⋮⋮</span> 三、组织实施
              </div>
              <div className="mb-2.5 flex items-center gap-[18px] rounded-lg px-[11px] py-[9px] text-[0.98rem] text-ink-dim">
                <span className="font-mono text-[0.85rem] text-ink-dim opacity-50">⋮⋮</span> 四、工作要求
              </div>
            </div>
            <div className="flex-1 p-9">
              <div className="mb-3.5 mt-7 font-mono text-[0.76rem] uppercase tracking-[0.08em] text-ink-dim">
                本段要点
              </div>
